<template>
  <div>
    ...
    <h3>Canli Yayin</h3>
    <button type="button" @click="startLive()">{{liveActive ? 'Canli Yayin Aktif' : 'Canli Yayini Baslat'}}</button>
    <br>
    <input type="range" min="0" max="100" v-model="liveVolumeLevel" @change="changeLiveVolume()" @input="changeLiveVolume()">

    <div v-if="liveActive">
        <table v-for="(cheerGroupIds, type) in cheerGroups" :key="type">
            <h3>{{type}}</h3>
            <button @click="changeMuteLiveType(type)">{{liveTypeMuted[type] ? 'Sesleri Ac' : 'Sesleri Kapat'}}</button>
            <ul v-for="cheerGroupId in cheerGroupIds" :key="cheerGroupId">
                <span>{{cheerGroupId}}:  </span>
                <input type="range" min="0" max="100" v-model="liveGroupsVolumeLevels[cheerGroupId]" @change="changeLiveGroupVolume(cheerGroupId, liveGroupsVolumeLevels[cheerGroupId])" @input="changeLiveGroupVolume(cheerGroupId, liveGroupsVolumeLevels[cheerGroupId])">
            </ul>
        </table>
    </div>
...
export default {
  data () {
    return {
      sounds: {'Mars': [{'name':'12 Dev Adam', 'fileName':'12DevAdam.m4a'}, {'name':'12 Dev Adam 2', 'fileName':'12DevAdam_2.m4a'}],
        'Tezahurat': [{'name':'Sari - Kirmizi', 'fileName':'sarikirmizi.m4a'}, {'name':'Sari - Kirmizi Stereo', 'fileName':'sarikirmizi_stereo.m4a'}, {'name':'Kirmizi - Beyaz', 'fileName':'kirmizibeyaz.m4a'}, {'name':'Kirmizi - Beyaz Stereo', 'fileName':'kirmizibeyaz_stereo.m4a'}],
        'Alkis': [{'name':'Alkis', 'fileName':'alkis.m4a'}, {'name':'Alkis2', 'fileName':'alkis2.m4a'}, {'name':'Alkis4', 'fileName':'alkis4.m4a'}, {'name':'Alkis6', 'fileName':'alkis6.m4a'}, {'name':'Alkis8', 'fileName':'alkis8.m4a'}, {'name':'Alkis10', 'fileName':'alkis10.m4a'}],
        'Islik': [{'name':'Islik', 'fileName':'islik.m4a'}, {'name':'Islik2', 'fileName':'islik2.m4a'}, {'name':'Islik4', 'fileName':'islik4.m4a'}, {'name':'Islik6', 'fileName':'islik6.m4a'}, {'name':'Islik8', 'fileName':'islik8.m4a'}, {'name':'Islik10', 'fileName':'islik10.m4a'}],
        'Yuh': [{'name':'Yuh', 'fileName':'yuh.m4a'}, {'name':'Yuh2', 'fileName':'yuh2.m4a'}, {'name':'Yuh4', 'fileName':'yuh4.m4a'}, {'name':'Yuh6', 'fileName':'yuh6.m4a'}, {'name':'Yuh8', 'fileName':'yuh8.m4a'}, {'name':'Yuh10', 'fileName':'yuh10.m4a'}],
        'Gurultu': [{'name':'Crowd Football', 'fileName':'crowd_football.m4a'}, {'name':'Crowd Football 2', 'fileName':'crowd_football2.m4a'}]
      },
      soundsVolumeLevels: {},
      soundsAudios: {},
  ...
  methods: {
    onChangeSoundVolume(fileName){
        this.soundsAudios[fileName].volume = this.soundsVolumeLevels[fileName]/100;
    },
    playSound (fileName) {
    ...
    getNextLiveCheer() {
        ...
        if (this.liveActive) {
            axios.get("/cheer/next")
            .then((response) => { 
                this.nextSounds = [];
                var soundsArray = response.data;
                for (let i = 0; i < soundsArray.length; ++i) { //i++ ???????
    ...
    processNextSounds() {
        if (this.activeLiveAudios.length == 0) { //his canli ses yoksa
            console.log('----------List bos oynat---------');
            this.playNextSound();
        } else if (!this.areArraysEqual() && (this.activeLiveAudios[0].audio.duration - this.activeLiveAudios[0].audio.currentTime) < 5) { //5. snden sonra ses dosyasi degisirse
            console.log('NEW sounds');
            clearTimeout(this.playNextSoundTimeout);
            clearTimeout(this.loadNextSoundTimeout);
            this.activeLiveAudios.forEach(map => { //active sound objelerine dokunmadan sadece audiolar ustunden. active soundlar degisecek.
                setTimeout(() => {map.audio.pause(); console.log('FADEOUT sound:' + map.fileName);}, 1000);
                
            });
            this.playNextSound();
        } else { //ses dosyasi degismiyorsa
            console.log('Liste dolu bekle nextSounds:' + this.nextSounds.length);
            var startDelay = (this.activeLiveAudios[0].audio.duration - this.activeLiveAudios[0].audio.currentTime) * 1000;
            console.log('delay:' + startDelay);
            clearTimeout(this.playNextSoundTimeout);
            clearTimeout(this.loadNextSoundTimeout);
            this.loadNextSoundTimeout = setTimeout(() => this.loadNextSound(), startDelay - 250);
            this.playNextSoundTimeout = setTimeout(() => this.playNextSound(), startDelay - 1000); //suankiler bittikten sonra
        }
    ...
    changeMuteLiveType(type) {
        ...
        for (let i = 0; i < this.cheerGroups[type].length; i++) {
            ...
            if (this.liveTypeMuted[type]) {
                this.lastLiveGroupsVolumeLevels[cheerGroupId] = this.liveGroupsVolumeLevels[cheerGroupId];
            }
            console.log(cheerGroupId + ' - ' + this.liveTypeMuted[type] + ' - ' + this.lastLiveGroupsVolumeLevels[cheerGroupId] + ' - result:' + (this.liveTypeMuted[type] ? 0 : this.lastLiveGroupsVolumeLevels[cheerGroupId]));
            this.changeLiveGroupVolume(cheerGroupId , this.liveTypeMuted[type] ? 0 : this.lastLiveGroupsVolumeLevels[cheerGroupId]);
        }
    },
    changeLiveGroupVolume(groupId, value) {
        console.log(groupId + ' - ' + value);
        this.liveGroupsVolumeLevels[groupId] = value
        console.log('liveGroupsVolumeLevels:' + this.liveGroupsVolumeLevels[groupId] + ' groupId:' + groupId);
        this.activeLiveAudios.forEach(map => {
