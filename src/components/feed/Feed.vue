<template>
  <div>
    ...
    <div v-for="(gameFeedArray, feedIndex) in feed" :key="feedIndex">
        <ul v-for="(gameFeed, gameFeedIndex) in gameFeedArray" :key="gameFeedIndex">
            <li>
                ...
                <br>
                <button @click="addSounds(gameFeed.text, gameFeed.sounds)">Play</button>
...
export default {
  ...
  methods: {
    ...
    addSounds(name, sounds){
      eventBus.$emit('addTrack-Player', {'name': name, 'sounds': sounds});
    },
    getFeed() {
      axios.get("/games/1/feed")
      .then((response) => { 
